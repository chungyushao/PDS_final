...
def main():
	...
	for year in years:
		...
		root = ET.parse(requests.get("http://www.basketball-reference.com/leagues/NBA_" + year + ".html").content)
		for i in range(1, 31):
			abr_regex = re.compile("^\/teams\/(.*)\/.*\.html");
			print root.findall(".//table[@id='team-stats-per_game_clone]/tbody/tr[0]/td[0]/a/@href")
			team_page_link = season_summary.xpath('//table[@id="team"]/tbody/tr[' + str(i) + ']/td[0]/a/@href')[0];
			break
			# print root.findall(".//table[@id='team-stats-per_game_clone]/tbody/tr[' + str(i) + ']/td[0]/a/@href")
			# print root.findall(".//table[@id='team-stats-per_game_clone]/tbody/tr[0]/td[0]/a/@href")
